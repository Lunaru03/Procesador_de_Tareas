...
int procesador_de_tareas(struct elista** inicio, struct cola* cola, struct pila* pila, int n)
{
    int np, bandera; //np= numero de veces que se generan procesos.
    ...
    while(np>0  || !cola_vacia(cola) || hilos_ocupados(inicio))
    {
        ...
        hilos_trabajando(inicio, pila, n);
        system("pause");
        system("cls");
...
int hilos_trabajando(struct elista ** lista, struct pila* pila, int n)
{
    ...
    {
        int ID=genera_numero(1, n);
        printf("vamos a liberar el hilo %d\n", ID);
        aux=libera_hilox(lista, ID);
        if(aux)
        inserta_epila(pila, aux);
    }
    return 1;
}
